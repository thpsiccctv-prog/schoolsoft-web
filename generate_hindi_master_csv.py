...
# Comprehensive dictionary for Purvanchal / UP Board names
DICTIONARY = {
    ...
    # Ambiguous Gender Names -> Set to MEDIUM and provide context-aware suggestions
    ...
    "NEETU": ("नीतू", "HIGH", ""),
    "NEHA": ("नेहा", "HIGH", ""),
    "NESHA": ("निशा", "MEDIUM", "Spelling Nesha->निशा / नेशा"),
    "NIDHI": ("निधि", "HIGH", ""),
    "NIKHIL": ("निखिल", "HIGH", ""),
    "NIKITA": ("निकिता", "HIGH", ""),
    "NIRAJ": ("नीरज", "HIGH", ""),
    ...
    "SHADAT": ("सआदत", "LOW", "Muslim/Urdu name"),
...
# Rule-based fallback transliterator
CONSONANTS = [
...
def transliterate_name(full_name_eng, context_type="student"):
    ...
    if "KUMARI" in words_upper or "DEVI" in words_upper:
        effective_context = "female"
...
def generate_master_files():
    ...
    # 1. Write CSV with UTF-8 BOM (if not locked by Excel)
    try:
        ...
        print("Note: CSV file is currently open in Excel. Writing XLSX file instead.")
    ...
    print(f"Master CSV created: {output_csv_path}")
    print(f"Master XLSX created: {output_xlsx_path}")
